...
OMX_ERRORTYPE dxb_omx_isdbt_tuner_tcc353x_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) 
{
	...
	{
		openmaxStandComp->pComponentPrivate = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_isdbt_tuner_tcc353x_component_PrivateType));
	...
	omxErr = dxb_omx_base_source_Constructor(openmaxStandComp,cComponentName);
	...
	{
		omx_isdbt_tuner_tcc353x_component_Private->ports = TCC_fo_calloc (__func__, __LINE__,omx_isdbt_tuner_tcc353x_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts, sizeof(omx_base_audio_PortType *));
		...
		{
			omx_isdbt_tuner_tcc353x_component_Private->ports[i] = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_base_audio_PortType));
	...
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_isdbt_tuner_tcc353x_component_Private->ports[0], 0, OMX_FALSE);

	pPort = (omx_base_audio_PortType *) omx_isdbt_tuner_tcc353x_component_Private->ports[OMX_BASE_SOURCE_OUTPUTPORT_INDEX];
	omx_isdbt_tuner_tcc353x_component_Private->BufferMgmtCallback = dxb_omx_isdbt_tuner_tcc353x_component_BufferMgmtCallback;
	omx_isdbt_tuner_tcc353x_component_Private->messageHandler = dxb_omx_isdbt_tuner_tcc353x_component_MessageHandler;	
	omx_isdbt_tuner_tcc353x_component_Private->destructor = dxb_omx_isdbt_tuner_tcc353x_component_Destructor;

	openmaxStandComp->SetParameter  = dxb_omx_isdbt_tuner_tcc353x_component_SetParameter;
	openmaxStandComp->GetParameter  = dxb_omx_isdbt_tuner_tcc353x_component_GetParameter;

	openmaxStandComp->SetConfig = dxb_omx_isdbt_tuner_tcc353x_component_SetConfig;
	openmaxStandComp->GetConfig = dxb_omx_isdbt_tuner_tcc353x_component_GetConfig;

	openmaxStandComp->GetExtensionIndex = dxb_omx_isdbt_tuner_tcc353x_component_GetExtensionIndex;
...
OMX_ERRORTYPE dxb_omx_isdbt_tuner_tcc353x_component_Destructor(OMX_COMPONENTTYPE *openmaxStandComp)
{
	...
	if (omx_isdbt_tuner_tcc353x_component_Private->ports) 
	{
		...
		TCC_fo_free (__func__, __LINE__,omx_isdbt_tuner_tcc353x_component_Private->ports);
		omx_isdbt_tuner_tcc353x_component_Private->ports=NULL;
	}
...
void dxb_omx_isdbt_tuner_tcc353x_component_BufferMgmtCallback(OMX_COMPONENTTYPE *openmaxStandComp, OMX_BUFFERHEADERTYPE* outputbuffer) 
{
	OMX_S32 data_read;
	omx_isdbt_tuner_tcc353x_component_PrivateType* omx_isdbt_tuner_tcc353x_component_Private = openmaxStandComp->pComponentPrivate;
	int cnt;
...
char *pCallbackString = "OMX TCC353X STRING";
	...
	*/
	...
	{		
		...
		case OMX_IndexVendorParamTunerChannelSet:		
			...
			piArg = (int *)piArg[1];
			channel = piArg[0];
			err = tcc353x_frequency_get (channel, (I32S *)&freq);
			if (err == 0) {
...
#if 0
			/* it's a sample code to issue a custom event */

			(*(omx_isdbt_tuner_tcc353x_component_Private->callbacks->EventHandler)) (openmaxStandComp,
			   omx_isdbt_tuner_tcc353x_component_Private->callbackData,
			   OMX_EventVendorSpecificEvent,
			   OMX_VENDOR_EVENTDATA_TUNER, 0, pCallbackString);
#endif
			break;
		...
		case OMX_IndexVendorParamTunerSetCasSetPid:
			...
			Tcc353xApiCasSetPid  (0, (I32U *)&puiArg[1], puiArg[0]);
		break;
		...
		case OMX_IndexVendorParamRegisterPID:
			//empty in intention.
			break;
		case OMX_IndexVendorParamTunerCustom:
		{
			piArg = (int *)ComponentParameterStructure;
			int iSample = *(int*)piArg[1];
			int iSize = (int)piArg[0];
			ALOGE("%s::OMX_IndexVendorParamTunerCustom Sample=%d Size=%d\n", __func__, iSample, iSize);
			break;
		}
		case OMX_IndexVendorParamTunerUserLoopStopCmd:
		{
			int moduleIndex = (int)ComponentParameterStructure;
			tcc353x_UserLoopStopCmd(moduleIndex);
  ...
  OMX_INOUT OMX_PTR ComponentParameterStructure)
{
	...
	{		
		case OMX_IndexVendorParamTunerChannelSet:						
			{
				...
				pSearchInfo =(TUNER_SEARCH_INFO *)piArg[1];
		...
		case OMX_IndexVendorParamGetSignalStrength:
			{
				...
				puiStr = (unsigned int *)piArg[1];
				_Tcc353xStatusToUi *pStatToUi;
				tmccInfo_t TmccInfo = { 0, };
		...
		case OMX_IndexVendorParamGetSignalStrengthIndex:
			{
				int *puiStr;
				Tcc353xStatus_t monitorValues;
		...
		case OMX_IndexVendorParamGetSignalStrengthIndexTime:
			{
		...
		case OMX_IndexVendorParamTunerCustom:
			{
				int *piArg = (int *)ComponentParameterStructure;
				*(int *)piArg[0] = sizeof(int);
				*(int *)piArg[1] = 2;
				break;
			}
		case OMX_IndexVendorParamTunerGetLockStatus:
			{
				int *lock_status = (int*)ComponentParameterStructure;
				I32S arg;
				ret = tcc353x_get_lock_status((I32S *)&arg);
				if (ret==0) {
					*lock_status = 1; 				//It's for future use
					err = OMX_ErrorNone;			//Lock success
				} else {
					*lock_status = 0;				//it's for future use.
					err = OMX_ErrorBadParameter;	//Lock fail
					//err = OMX_ErrorNotReady;		//be trying to lock
				}
				break;
			}
		default: /*Call the base component function*/
...
OMX_ERRORTYPE dxb_omx_isdbt_tuner_tcc353x_component_MessageHandler(OMX_COMPONENTTYPE* openmaxStandComp, internalRequestMessageType *message)
{
...
{
	return (dxb_omx_isdbt_tuner_tcc353x_component_Constructor(openmaxStandComp, BROADCAST_TCC353X_I2C_STS_TUNER_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc353x_CSPI_STS_tuner_component_Init (OMX_COMPONENTTYPE *openmaxStandComp) 
{
	return (dxb_omx_isdbt_tuner_tcc353x_component_Constructor(openmaxStandComp, BROADCAST_TCC353X_CSPI_STS_TUNER_NAME));
}

OMX_ERRORTYPE dxb_omx_tcc353x_I2C_STS_tuner_component_Deinit(OMX_COMPONENTTYPE *openmaxStandComp)
{
	return dxb_omx_isdbt_tuner_tcc353x_component_Destructor(openmaxStandComp);
}
