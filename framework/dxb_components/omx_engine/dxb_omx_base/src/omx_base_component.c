...
*   Description : omx_base_component.c
...
#define VIDEO_SINK_NAME		"OMX.tcc.fbdev.fbdev_sink"
#define AUDIO_SINK_NAME  		"OMX.tcc.alsa.alsasink"

#if defined(HAVE_LINUX_PLATFORM)
#define LOG_TAG	"OMX_Base_Component"
#include <utils/Log.h>
...
#define DBug_MSG(msg...)	if (debug) { ALOGD( ": " msg);/* sleep(1);*/}
#else
#define DBug_MSG(msg...)	printf(msg)
#endif

#define STACK_SIZE				64*1024
#define THREAD_SCHED_TYPE		SCHED_RR
#define SINK_COMP_PRIO			1
#define OTHER_COMP_PRIO			1
/**
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
  ...
  if (openmaxStandComp->pComponentPrivate) {
    ...
    omx_base_component_Private = TCC_fo_calloc (__func__, __LINE__,1,sizeof(omx_base_component_PrivateType));
  ...
  if(!omx_base_component_Private->messageQueue) {
    omx_base_component_Private->messageQueue = TCC_fo_calloc (__func__, __LINE__,1,sizeof(queue_t));
    if (!omx_base_component_Private->messageQueue) {
      return OMX_ErrorInsufficientResources;
    }
    dxb_queue_init(omx_base_component_Private->messageQueue);
  }

  if(!omx_base_component_Private->messageSem) {
    omx_base_component_Private->messageSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if (!omx_base_component_Private->messageSem) {
      return OMX_ErrorInsufficientResources;
    }
  ...
  if(!omx_base_component_Private->bMgmtSem) {
    omx_base_component_Private->bMgmtSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if (!omx_base_component_Private->bMgmtSem) {
      return OMX_ErrorInsufficientResources;
    }
  ...
  if(!omx_base_component_Private->bStateSem) {
    omx_base_component_Private->bStateSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if (!omx_base_component_Private->bStateSem) {
      return OMX_ErrorInsufficientResources;
    }
	...
	if(!omx_base_component_Private->bPortChangeSem) {
		omx_base_component_Private->bPortChangeSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
		if (!omx_base_component_Private->bPortChangeSem) {
      		return OMX_ErrorInsufficientResources;
    	}
		tsem_init(omx_base_component_Private->bPortChangeSem, 0);
	}

  openmaxStandComp->nSize = sizeof(OMX_COMPONENTTYPE);
  openmaxStandComp->pApplicationPrivate = NULL;
  openmaxStandComp->GetComponentVersion = dxb_omx_base_component_GetComponentVersion;
  openmaxStandComp->SendCommand = dxb_omx_base_component_SendCommand;
  openmaxStandComp->GetParameter = dxb_omx_base_component_GetParameter;
  openmaxStandComp->SetParameter = dxb_omx_base_component_SetParameter;
  openmaxStandComp->GetConfig = dxb_omx_base_component_GetConfig;
  openmaxStandComp->SetConfig = dxb_omx_base_component_SetConfig;
  openmaxStandComp->GetExtensionIndex = dxb_omx_base_component_GetExtensionIndex;
  openmaxStandComp->GetState = dxb_omx_base_component_GetState;
  openmaxStandComp->SetCallbacks = dxb_omx_base_component_SetCallbacks;
  openmaxStandComp->ComponentDeInit = dxb_omx_base_component_ComponentDeInit;
  openmaxStandComp->ComponentRoleEnum = dxb_omx_base_component_ComponentRoleEnum;
  openmaxStandComp->ComponentTunnelRequest =dxb_omx_base_component_ComponentTunnelRequest;

  /*Will make Specific port Allocate buffer call*/
  openmaxStandComp->AllocateBuffer = dxb_omx_base_component_AllocateBuffer;
  openmaxStandComp->UseBuffer = dxb_omx_base_component_UseBuffer;
  openmaxStandComp->UseEGLImage = dxb_omx_base_component_UseEGLImage;
  openmaxStandComp->FreeBuffer = dxb_omx_base_component_FreeBuffer;
  openmaxStandComp->EmptyThisBuffer = dxb_omx_base_component_EmptyThisBuffer;
  openmaxStandComp->FillThisBuffer = dxb_omx_base_component_FillThisBuffer;
  ...
  omx_base_component_Private->name = TCC_fo_calloc (__func__, __LINE__,1,OMX_MAX_STRINGNAME_SIZE);
  ...
  omx_base_component_Private->DoStateSet = &dxb_omx_base_component_DoStateSet;
  omx_base_component_Private->messageHandler = dxb_omx_base_component_MessageHandler;
  omx_base_component_Private->destructor = dxb_omx_base_component_Destructor;
  ...
  if(!omx_base_component_Private->flush_all_condition) {
    omx_base_component_Private->flush_all_condition = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if (!omx_base_component_Private->flush_all_condition) {
      	return OMX_ErrorInsufficientResources;
    }
  ...
  if(!omx_base_component_Private->flush_condition) {
    omx_base_component_Private->flush_condition = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if (!omx_base_component_Private->flush_condition) {
      	return OMX_ErrorInsufficientResources;
    }
  ...
  omx_base_component_Private->messageHandlerThreadID = TCCTHREAD_Create(&omx_base_component_Private->messageHandlerThread, (pThreadFunc)dxb_compMessageHandlerFunction,  														omx_base_component_Private->name, PREDEF_PRIORITY, openmaxStandComp);
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_Destructor(OMX_COMPONENTTYPE *openmaxStandComp) {
  ...
  if(omx_base_component_Private->messageQueue) {
    dxb_queue_deinit(omx_base_component_Private->messageQueue);
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->messageQueue);
    omx_base_component_Private->messageQueue=NULL;
  }

  err = TCCTHREAD_Join(omx_base_component_Private->messageHandlerThread,NULL);
  if(err!=0) {
  ...
  if(omx_base_component_Private->bMgmtSem){
    ...
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->bMgmtSem);
  ...
  if(omx_base_component_Private->messageSem) {
    ...
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->messageSem);
    omx_base_component_Private->messageSem=NULL;
  }

  if(omx_base_component_Private->bStateSem){
    tsem_deinit(omx_base_component_Private->bStateSem);
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->bStateSem);
    omx_base_component_Private->bStateSem=NULL;
  }

	if(omx_base_component_Private->bPortChangeSem){
		tsem_deinit(omx_base_component_Private->bPortChangeSem);
		TCC_fo_free (__func__, __LINE__,omx_base_component_Private->bPortChangeSem);
		omx_base_component_Private->bPortChangeSem=NULL;
	}

  if(omx_base_component_Private->name){
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->name);
  ...
  if(omx_base_component_Private->flush_all_condition){
    ...
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->flush_all_condition);
    omx_base_component_Private->flush_all_condition=NULL;
  }

  if(omx_base_component_Private->flush_condition){
    tsem_deinit(omx_base_component_Private->flush_condition);
    TCC_fo_free (__func__, __LINE__,omx_base_component_Private->flush_condition);
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_ComponentDeInit(
  ...
  TCC_fo_free (__func__, __LINE__,openmaxStandComp->pComponentPrivate);
 ...
 * a specific handling
 */
OMX_ERRORTYPE dxb_omx_base_component_DoStateSet(OMX_COMPONENTTYPE *openmaxStandComp, OMX_U32 destinationState) {
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)openmaxStandComp->pComponentPrivate;
  ...
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s\n", __func__);
  DEBUG(DEB_LEV_PARAMS, "at(%s) Changing state from %i to %i\n", omx_base_component_Private->name, omx_base_component_Private->state, (int)destinationState);
  ...
  if(destinationState == OMX_StateLoaded){
    ...
    case OMX_StateIdle:
        ...
        for(i = omx_base_component_Private->sPortTypesParam[j].nStartPortNumber;
          ...
          if (PORT_IS_TUNNELED(pPort) && PORT_IS_BUFFER_SUPPLIER(pPort)) {
            while(pPort->pBufferQueue->nelem > 0) {
              ...
              dxb_dequeue(pPort->pBufferQueue);
            }
          ...
          if(pPort->pInternalBufferStorage != NULL) {
            TCC_fo_free (__func__, __LINE__,pPort->pInternalBufferStorage);
            pPort->pInternalBufferStorage=NULL;
          }

          if(pPort->bBufferStateAllocated != NULL) {
            TCC_fo_free (__func__, __LINE__,pPort->bBufferStateAllocated);
      ...
      if(omx_base_component_Private->bufferMgmtThreadID == 0 ){
		...
		TCCTHREAD_Join(omx_base_component_Private->bufferMgmtThread,NULL);
        omx_base_component_Private->bufferMgmtThreadID = -1;
        if(err != 0) {
          DEBUG(DEB_LEV_FUNCTION_NAME,"In %s pthread_join returned err=%d\n",__func__,err);
        }
  ...
  if(destinationState == OMX_StateIdle){
	...
	omx_base_component_Private->bufferMgmtThreadID = TCCTHREAD_Create(&omx_base_component_Private->bufferMgmtThread, (pThreadFunc)omx_base_component_Private->BufferMgmtFunction,
  														omx_base_component_Private->name, PREDEF_PRIORITY, openmaxStandComp);
    ...
    case OMX_StateExecuting:
      ...
      for(j = 0; j < NUM_DOMAINS; j++) {
        ...
        i < omx_base_component_Private->sPortTypesParam[j].nStartPortNumber +
        omx_base_component_Private->sPortTypesParam[j].nPorts; i++) {
          DEBUG(DEB_LEV_FULL_SEQ, "Flushing Port %i (%s)\n",(int)i, omx_base_component_Private->name);
          pPort = omx_base_component_Private->ports[i];
          if(PORT_IS_ENABLED(pPort)) {
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_ParameterSanityCheck(OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_GetComponentVersion(OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_ComponentRoleEnum(
  OMX_IN OMX_HANDLETYPE hComponent,
 ...
 * See OMX_Component.h
 */
OMX_ERRORTYPE dxb_omx_base_component_SetCallbacks(
  OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_GetParameter(
  OMX_IN  OMX_HANDLETYPE hComponent,
  ...
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s\n", __func__);
  DEBUG(DEB_LEV_PARAMS, "Getting parameter 0x%x\n", nParamIndex);
  ...
  case OMX_IndexVendorCompPropTunnelFlags:
    ...
    break;
  default:
    DEBUG(DEB_LEV_ERR, "OMX_ErrorUnsupportedIndex\n");
 ...
 * @return OMX_ErrorUnsupportedIndex if the index is not supported or not handled here
 */
OMX_ERRORTYPE dxb_omx_base_component_SetParameter(
  OMX_IN  OMX_HANDLETYPE hComponent,
  OMX_IN  OMX_INDEXTYPE nParamIndex,
  ...
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s\n", __func__);
  DEBUG(DEB_LEV_PARAMS, "Setting parameter 0x%x\n", nParamIndex);
  if (ComponentParameterStructure == NULL) {
  ...
  case OMX_IndexParamPortDefinition:
    pPortDef  = (OMX_PARAM_PORTDEFINITIONTYPE*) ComponentParameterStructure;
    err = dxb_omx_base_component_ParameterSanityCheck(hComponent, pPortDef->nPortIndex, pPortDef, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
    if(err!=OMX_ErrorNone) {
    ...
    }
    {
      ...
      if ((omx_base_component_Private->state == OMX_StateIdle ||
        ...
        if(pPort->pInternalBufferStorage) {
          pPort->pInternalBufferStorage = TCC_realloc(pPort->pInternalBufferStorage,pPort->sPortParam.nBufferCountActual*sizeof(OMX_BUFFERHEADERTYPE *));
        }

        if(pPort->bBufferStateAllocated) {
          pPort->bBufferStateAllocated = TCC_realloc(pPort->bBufferStateAllocated,pPort->sPortParam.nBufferCountActual*sizeof(BUFFER_STATUS_FLAG));
          if(!pPort->bBufferStateAllocated){
          	if(pPort->pInternalBufferStorage){
          		TCC_fo_free(__func__, __LINE__, pPort->pInternalBufferStorage);
          	}
  ...
  case OMX_IndexParamCompBufferSupplier:
	...
	*/
    ...
    err = dxb_omx_base_component_ParameterSanityCheck(hComponent, pBufferSupplier->nPortIndex, pBufferSupplier, sizeof(OMX_PARAM_BUFFERSUPPLIERTYPE));
  ...
  default:
	...
	if(err != OMX_ErrorNone)
		ALOGE("ERROR %s(%s) :: nParamIndex = 0x%x, error(0x%x)", __func__, omx_base_component_Private->name, nParamIndex, err);
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_GetConfig(
  OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 * in the private component descriptor
 */
OMX_ERRORTYPE dxb_omx_base_component_SetConfig(
  OMX_IN  OMX_HANDLETYPE hComponent,
  OMX_IN  OMX_INDEXTYPE nIndex,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_GetExtensionIndex(
  OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_GetState(
  OMX_IN  OMX_HANDLETYPE hComponent,
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_SendCommand(
  OMX_IN  OMX_HANDLETYPE hComponent,
  ...
  message = TCC_fo_calloc (__func__, __LINE__,1,sizeof(internalRequestMessageType));
  if(!message){
  	return OMX_ErrorInsufficientResources;
  }
    ...
    if ((nParam == OMX_StateIdle) && (omx_base_component_Private->state == OMX_StateLoaded)) {
      ...
      for(j = 0; j < NUM_DOMAINS; j++) {
        for(i = omx_base_component_Private->sPortTypesParam[j].nStartPortNumber;
          ...
          if(pPort->pInternalBufferStorage == NULL) {
            pPort->pInternalBufferStorage = TCC_fo_calloc (__func__, __LINE__,pPort->sPortParam.nBufferCountActual,sizeof(OMX_BUFFERHEADERTYPE *));
            if(!pPort->pInternalBufferStorage){
            	return OMX_ErrorInsufficientResources;
            }
          }
          ...
          if(pPort->bBufferStateAllocated == NULL) {
            pPort->bBufferStateAllocated = TCC_fo_calloc (__func__, __LINE__,pPort->sPortParam.nBufferCountActual,sizeof(BUFFER_STATUS_FLAG));
            if(!pPort->bBufferStateAllocated){
            	return OMX_ErrorInsufficientResources;
            }
          }
  ...
  case OMX_CommandFlush:
    if (nParam >= (omx_base_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts +
		...
		/* jini 14th : Redundant Condition
		if(message){
      		TCC_fo_free(__func__, __LINE__, message);
      	}
      	*/
      	TCC_fo_free(__func__, __LINE__, message);
  ...
  if (err == OMX_ErrorNone)
  {
    dxb_queue(messageQueue, message);
    tsem_up(messageSem);
  }
 ...
 */
void* dxb_compMessageHandlerFunction(void* param) {
  ...
  while(1){
    ...
    DEBUG(DEB_LEV_FUNCTION_NAME, "In %s (%s)\n", __func__, omx_base_component_Private->name);
    /*Destructor has been called. So exit from the loop*/
    if(omx_base_component_Private->state == OMX_StateInvalid) {
      break;
    }
    /* Dequeue it */
    message = dxb_dequeue(omx_base_component_Private->messageQueue);
    if(message == NULL){
 ...
 */
OMX_ERRORTYPE dxb_omx_base_component_MessageHandler(OMX_COMPONENTTYPE *openmaxStandComp,internalRequestMessageType* message) {
  ...
  */
  ...
  case OMX_CommandStateSet: {
    ...
    if (err != OMX_ErrorNone) {
      ...
      DEBUG(DEB_LEV_FULL_SEQ,"running callback in %s (%s)\n",__func__, omx_base_component_Private->name);
      (*(omx_base_component_Private->callbacks->EventHandler))
      (openmaxStandComp,
      omx_base_component_Private->callbackData,
...
OMX_ERRORTYPE dxb_omx_base_component_AllocateBuffer(
            OMX_IN OMX_HANDLETYPE hComponent,
...
OMX_ERRORTYPE dxb_omx_base_component_UseBuffer(
            OMX_IN OMX_HANDLETYPE hComponent,
...
OMX_ERRORTYPE dxb_omx_base_component_UseEGLImage (
...
OMX_ERRORTYPE dxb_omx_base_component_FreeBuffer(
            OMX_IN  OMX_HANDLETYPE hComponent,
            OMX_IN  OMX_U32 nPortIndex,
...
OMX_ERRORTYPE dxb_omx_base_component_EmptyThisBuffer(
            OMX_IN  OMX_HANDLETYPE hComponent,
            OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer) {
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
  omx_base_PortType *pPort;
...
OMX_ERRORTYPE dxb_omx_base_component_FillThisBuffer(
  OMX_IN  OMX_HANDLETYPE hComponent,
  OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer) {

  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
  omx_base_PortType *pPort;
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s (%s) index:%d\n", __func__, omx_base_component_Private->name, pBuffer->nOutputPortIndex);
...
OMX_ERRORTYPE dxb_omx_base_component_ComponentTunnelRequest(
  OMX_IN  OMX_HANDLETYPE hComponent,
...
#if defined(HAVE_LINUX_PLATFORM)
OMX_ERRORTYPE dxb_omx_base_component_AllocateBuffer2(
            OMX_IN OMX_HANDLETYPE hComponent,
            OMX_INOUT OMX_BUFFERHEADERTYPE** ppBuffer,
            OMX_IN OMX_U32 nPortIndex,
            OMX_IN OMX_PTR pAppPrivate,
  ...
  return pPort->Port_AllocateBuffer2(pPort,
                                    ppBuffer,
                                    nPortIndex,
                                    pAppPrivate,
                                    nSizeBytes);
...
OMX_ERRORTYPE dxb_omx_base_component_UseBuffer2(
            OMX_IN OMX_HANDLETYPE hComponent,
            OMX_INOUT OMX_BUFFERHEADERTYPE** ppBufferHdr,
            OMX_IN OMX_U32 nPortIndex,
            OMX_IN OMX_PTR pAppPrivate,
  ...
  return  pPort->Port_UseBuffer2(pPort,
                                ppBufferHdr,
                                nPortIndex,
                                pAppPrivate,
                                nSizeBytes,
...
OMX_ERRORTYPE dxb_omx_base_component_FreeBuffer2(
            OMX_IN  OMX_HANDLETYPE hComponent,
            OMX_IN  OMX_U32 nPortIndex,
            OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer) {
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
  ...
  return pPort->Port_FreeBuffer2(pPort,
                                nPortIndex,
                                pBuffer);
}
#endif
