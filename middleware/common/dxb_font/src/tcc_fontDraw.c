...
int tcc_fontDraw_drawHighlighting(unsigned int uiX, unsigned int uiY, unsigned int uiWidth, unsigned int uiHeight, unsigned int uiVspace, unsigned int Type)
{
	...
	if (g_uiDirection == TCC_FONT_DIRECTION_HORIZONTAL) {
		if( Type & 0x1 ){
			puiDestBuf  = g_puiDestBuf + ((uiY + (uiHeight + uiVspace - TCC_FONT_HIGHLIGHT_CORRECTION)) * uiDestPitch) + uiX;
			for(i=0; i<uiWidth; i++)
			{
		...
		if( Type & 0x8 ){
			puiDestBuf  = g_puiDestBuf + (uiY * uiDestPitch) + uiX;	
			for(i=0; i<(uiHeight + uiVspace - TCC_FONT_HIGHLIGHT_CORRECTION); i++)
			{
				*puiDestBuf = uiDestFTColor;	
		...
		if( Type & 0x2 ){
			...
			puiDestBuf  = g_puiDestBuf + ((uiY + (uiHeight + uiVspace - TCC_FONT_HIGHLIGHT_CORRECTION)) * uiDestPitch) + uiX + (uiWidth - TCC_FONT_HIGHLIGHT_CORRECTION);	
			for(i=0; i<TCC_FONT_HIGHLIGHT_CORRECTION; i++)
			{
				*puiDestBuf++ = uiDestBGColor;	
			}
...
int tcc_fontDraw_drawUnderline(unsigned int uiX, unsigned int uiY, unsigned int uiWidth, unsigned int uiHeight, unsigned int uiVspace)
{
	...
	if (g_uiDirection == TCC_FONT_DIRECTION_HORIZONTAL) {
		puiDestBuf  = g_puiDestBuf + (((uiHeight + uiVspace - 1) + uiY) * uiDestPitch) + uiX;	
		for(i=0; i<uiWidth; i++)
		{
			*puiDestBuf++ = uiDestFTColor;	
		}
