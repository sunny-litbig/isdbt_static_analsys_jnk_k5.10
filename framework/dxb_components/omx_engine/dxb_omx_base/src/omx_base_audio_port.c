...
*   Description : omx_base_audio_port.c
...
OMX_ERRORTYPE dxb_base_audio_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  
  omx_base_audio_PortType *omx_base_audio_Port;

  if (!(*openmaxStandPort)) {
    *openmaxStandPort = TCC_fo_calloc (__func__, __LINE__,1,sizeof (omx_base_audio_PortType));
  }
  ...
  dxb_base_port_Constructor(openmaxStandComp,openmaxStandPort,nPortIndex, isInput);
  ...
  omx_base_audio_Port->sPortParam.format.audio.cMIMEType = TCC_fo_malloc(__func__, __LINE__,DEFAULT_MIME_STRING_LENGTH);
  ...
  omx_base_audio_Port->sPortParam.nBufferCountActual = 4;

  omx_base_audio_Port->PortDestructor = &dxb_base_audio_port_Destructor;
...
OMX_ERRORTYPE dxb_base_audio_port_Destructor(omx_base_PortType *openmaxStandPort){

  if(openmaxStandPort->sPortParam.format.audio.cMIMEType) {
    TCC_fo_free (__func__, __LINE__,openmaxStandPort->sPortParam.format.audio.cMIMEType);
    openmaxStandPort->sPortParam.format.audio.cMIMEType = NULL;
  }

  dxb_base_port_Destructor(openmaxStandPort);
