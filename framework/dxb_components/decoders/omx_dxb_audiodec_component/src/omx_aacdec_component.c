...
*   Description : omx_aacdec_component.c
...
OMX_ERRORTYPE dxb_OMX_DXB_AudioDec_AACCommonComponentInit(OMX_COMPONENTTYPE *openmaxStandComp)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;

	err = dxb_omx_aacdec_component_Constructor(openmaxStandComp,AUDIO_DEC_AAC_NAME, AUDIO_SUBTYPE_NONE);

	return err;
}

OMX_ERRORTYPE dxb_OMX_DXB_AudioDec_AACComponentInit(OMX_COMPONENTTYPE *openmaxStandComp)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;

	err = dxb_omx_aacdec_component_Constructor(openmaxStandComp,AUDIO_DEC_AAC_NAME, AUDIO_SUBTYPE_AAC_ADTS);

	return err;
}

OMX_ERRORTYPE dxb_OMX_DXB_AudioDec_AACLATM_ComponentInit(OMX_COMPONENTTYPE *openmaxStandComp)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;

	err = dxb_omx_aacdec_component_Constructor(openmaxStandComp,AUDIO_DEC_AAC_NAME, AUDIO_SUBTYPE_AAC_LATM);
...
OMX_ERRORTYPE dxb_omx_aacdec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName, int iAACType)
{
	...
	{
		openmaxStandComp->pComponentPrivate = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_aacdec_component_PrivateType));
	...
	*/
	...
	omx_aacdec_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts = 4;

	if (omx_aacdec_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts && !omx_aacdec_component_Private->ports)
	{
		omx_aacdec_component_Private->ports = TCC_fo_calloc (__func__, __LINE__,omx_aacdec_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts, sizeof(omx_base_PortType *));
		...
		{
			omx_aacdec_component_Private->ports[i] = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_base_audio_PortType));
			if (!omx_aacdec_component_Private->ports[i])
			{
				if(omx_aacdec_component_Private->ports){
		  			TCC_fo_free(__func__, __LINE__, omx_aacdec_component_Private->ports);
					omx_aacdec_component_Private->ports=NULL;
		  		}
				return OMX_ErrorInsufficientResources;
			}
	...
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_aacdec_component_Private->ports[0], 0, OMX_TRUE); // input
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_aacdec_component_Private->ports[1], 1, OMX_TRUE); // input
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_aacdec_component_Private->ports[2], 2, OMX_FALSE); // output
	dxb_base_audio_port_Constructor(openmaxStandComp, &omx_aacdec_component_Private->ports[3], 3, OMX_FALSE); // output
	...
	outPort = (omx_base_audio_PortType *) omx_aacdec_component_Private->ports[3];
	outPort->sPortParam.nBufferSize = AUDIO_DXB_OUT_BUFFER_SIZE;
	outPort->sPortParam.format.audio.eEncoding = OMX_AUDIO_CodingPCM;
	outPort->sAudioParam.eEncoding = OMX_AUDIO_CodingPCM;
	outPort->sPortParam.nBufferCountMin = 8;
	...
	omx_aacdec_component_Private->BufferMgmtFunction = dxb_omx_audiodec_twoport_filter_component_BufferMgmtFunction;
	omx_aacdec_component_Private->BufferMgmtCallback = dxb_omx_audiodec_component_BufferMgmtCallback;
	omx_aacdec_component_Private->messageHandler = dxb_omx_audiodec_component_MessageHandler;
	omx_aacdec_component_Private->destructor = dxb_omx_audiodec_component_Destructor;
	...
	{
		omx_aacdec_component_Private->pAudioDecPrivate = TCC_fo_calloc (__func__, __LINE__,iTotalHandle, sizeof(OMX_AUDIO_DEC_PRIVATE_DATA));
		if(omx_aacdec_component_Private->pAudioDecPrivate == NULL)
		{
