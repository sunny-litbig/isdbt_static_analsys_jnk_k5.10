...
#define PRINTF DPRINTF
  #if defined(_TCC8960_)
    #define JPEG_BASE_REG    0x15300000
  #else
...
static int vpu_env_open(int jpeg_enc_fd)
{
	int vpu_reset = 0;
	int ret_val;
...
static void vpu_env_close(void)
{
	...
	if(jpu_clk_fd)
	{
		...
		{
			ALOGE("Err:: vpu(%s) close failed!", TCC_CLK_DEV_NAME);
		}
		dev_fd = -1;
	}
...
#if 0
#ifdef JPEG_ENCODE_FOR_MJPEG
int jpu_enc (int jpeg_enc_fd, jpu_conf_enc_t* pConfEnc )
{
...
//				_cdk_fwrite((void *)gsenc_output.m_BitstreamOut[VA], sizeof(unsigned char), gsenc_output.m_iBitstreamOutSize, fp_out);
//			else
			{
				...
				memcpy((void *)pConfEnc->m_outVirtHeaderBuffAddr, (void*)gsenc_output.m_BitstreamOut[VA], pConfEnc->m_outSizeOfTotalBuff);
			}
		}
	}
