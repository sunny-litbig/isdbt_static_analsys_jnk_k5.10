...
*   Description : omx_base_image_port.c
...
OMX_ERRORTYPE dxb_base_image_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  
  omx_base_image_PortType *omx_base_image_Port;

  if (!(*openmaxStandPort)) {
    *openmaxStandPort = TCC_fo_calloc (__func__, __LINE__,1,sizeof (omx_base_image_PortType));
  }
  ...
  omx_base_image_Port->sPortParam.format.image.cMIMEType = TCC_fo_malloc(__func__, __LINE__,DEFAULT_MIME_STRING_LENGTH);
  ...
  omx_base_image_Port->PortDestructor = &dxb_base_image_port_Destructor;
...
OMX_ERRORTYPE dxb_base_image_port_Destructor(omx_base_PortType *openmaxStandPort){

  if(openmaxStandPort->sPortParam.format.image.cMIMEType) {
    TCC_fo_free (__func__, __LINE__,openmaxStandPort->sPortParam.format.image.cMIMEType);
