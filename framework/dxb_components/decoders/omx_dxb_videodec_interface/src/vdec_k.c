...
#undef DPRINTF
#ifndef DPRINTF
#define DPRINTF(msg...)	  if (DEBUG_ON) { ALOGE( ": " msg);}
#endif
#undef DSTATUS
#ifndef DSTATUS
...
void dxb_vpu_update_sizeinfo(unsigned int format, unsigned int bps, unsigned int fps, unsigned int image_width, unsigned int image_height, void *pVdec)
{
	CONTENTS_INFO info;
	_vdec_ * pInst = pVdec;
...
static int dxb_vpu_check_for_video(unsigned char open_status, int handle, _vdec_ * pVdec)
{
//return value : if hw reset need or not!!
	...
	{
		sInfo.opened_cnt = 1;
		ioctl(pInst->vpu_mgr_fd, VPU_SET_MEM_ALLOC_MODE, &sInfo);
...
static int dxb_vpu_env_open(int handle, unsigned int format, unsigned int bps, unsigned int fps, unsigned int image_width, unsigned int image_height, _vdec_ *pVdec)
{
	int vpu_reset = 0;
	_vdec_ * pInst = pVdec;
	DSTATUS("In  %s \n",__func__);
...
// dxb_vpu_check_for_video returns always 0.
	dxb_vpu_check_for_video(1, handle, pInst);
#else
	if((vpu_reset = dxb_vpu_check_for_video(1, handle, pInst)) < 0)
		goto err;
#endif
	...
	dxb_vpu_update_sizeinfo(format, bps, fps, image_width, image_height, pInst);
...
err:
	DPRINTF("dxb_vpu_env_open error");
	dxb_vpu_env_close(handle, pInst);
...
static void dxb_vpu_env_close(int handle, _vdec_ *pVdec)
{
	...
	if( 0 > ioctl(pInst->vpu_dec_fd, V_DEC_FREE_MEMORY, NULL))
	{
		DPRINTF("vpu[%d] ioctl err: cmd = 0x%x", pInst->vdec_instance_index, V_DEC_FREE_MEMORY);
	}

	pInst->vdec_env_opened = 0;
	dxb_vpu_check_for_video(0, handle, pInst);
...
static int dxb_vdec_cmd_process(int cmd, void* args, _vdec_ *pVdec)
{
	int ret;
	int success = 0;
	_vdec_ * pInst = pVdec;
	...
	{
		DPRINTF("vpu[%d] ioctl err: cmd = 0x%x", pInst->vdec_instance_index, cmd);
	}
	...
	while (retry_cnt > 0) {
		...
		if (ret < 0) {
			DPRINTF("vpu[%d]-retry(%d) poll error", pInst->vdec_instance_index, retry_cnt);
			retry_cnt--;
			continue;
//			goto Retry;//break;
		}else if (ret == 0) {
...
void dxb_vpu_set_additional_frame_count(int count, void * pInst)
{
...
unsigned char *dxb_vpu_getBitstreamBufAddr(unsigned int index, void * pVdec)
{
...
unsigned char *dxb_vpu_getFrameBufVirtAddr(unsigned char *convert_addr, unsigned int base_index, void * pVdec)
{
...
int dxb_vpu_getBitstreamBufSize(void *pVdec)
{
	_vdec_ *pInst = pVdec;
	return pInst->gsBitstreamBufSize;
}

unsigned char *dxb_vpu_getUserDataBufAddr(unsigned int index, void * pInst)
{
...
int dxb_vpu_getUserDataBufSize(void *pInst)
{
	_vdec_ *pVdec = (_vdec_ *)pInst;
...
int
dxb_vpu_dec_ready( dec_init_t* psVDecInit, _vdec_ *pVdec )
{
	//------------------------------------------------------------
...
int
dxb_vpu_dec_seq_header( int iSize , dec_initial_info_t * psInitialInfo, _vdec_ *pVdec )
{
	int ret = 0;
	_vdec_ * pInst = pVdec;
//	DSTATUS("[VDEC-%d]dxb_vpu_dec_seq_header in :: size(%d), JpegOnly(%d), format(%d)", pInst->vdec_instance_index, iSize ,pInst->gsVpuDecUserInfo.m_bJpegOnly, pInst->gsVpuDecInit_Info.gsVpuDecInit.m_iBitstreamFormat);
	pInst->gsVpuDecSeqHeader_Info.stream_size = iSize;
	ret = dxb_vdec_cmd_process(V_DEC_SEQ_HEADER, &pInst->gsVpuDecSeqHeader_Info, pInst);
	...
	ret = dxb_vdec_cmd_process(V_DEC_REG_FRAME_BUFFER, &pInst->gsVpuDecBuffer_Info, pInst);
...
dxb_vdec_vpu( int iOpCode, int* pHandle, void* pParam1, void* pParam2, void* pParam3 )
{
	...
	if( iOpCode == VDEC_INIT )
	{
		...
		if(dxb_vpu_env_open(handle, p_input_param->m_iBitstreamFormat, 30, 30, 1920, 1080, pInst ) < 0)
		{
		...
		ret = dxb_vpu_dec_ready( &pInst->gsVpuDecInit_Info.gsVpuDecInit, pInst );
		if( ret != RETCODE_SUCCESS )
		{
			DPRINTF( "[VDEC-%d] ret : %02d, dxb_vpu_dec_ready error!!\n", pInst->vdec_instance_index, ret );
			return ret;
		}
	...
	#ifdef SUPPORT_VCACHE_CTRL
					pInst->gsVpuDecInit_Info.gsVpuDecInit.m_bEnableVideoCache,
	#else
					0,
	#endif
		...
		ret = dxb_vdec_cmd_process(V_DEC_INIT, &pInst->gsVpuDecInit_Info, pInst);
		if( ret != RETCODE_SUCCESS )
		{
		...
		ret = dxb_vdec_cmd_process(V_GET_VPU_VERSION, &pInst->gsVpuDecVersion, pInst);
		if( ret != RETCODE_SUCCESS )
		{
	...
	else if( iOpCode == VDEC_DEC_SEQ_HEADER )
	{
		...
		ret = dxb_vpu_dec_seq_header(seq_stream_size, NULL/*pParam2*/, pInst);
		if( ret != RETCODE_SUCCESS )
		{
			return ret;
		}
...
//		pInst->gsVpuDecInOut_Info.gsVpuDecInput.m_iUserDataBufferSize = p_input_param->m_iUserDataBufferSize;
		...
		ret = dxb_vdec_cmd_process(V_DEC_DECODE, &pInst->gsVpuDecInOut_Info, pInst);
		pInst->total_frm++;
...
//			return ERR_END_OF_FILE;
		...
		if(pInst->gsVpuDecInOut_Info.gsVpuDecOutput.m_DecOutInfo.m_iPicType == 0){
			//DSTATUS( "[VDEC-%d] I-Frame (%d)\n", pInst->vdec_instance_index, pInst->total_frm);
		}
		...
		p_output_param->m_pDispOut[VA][0] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pDispOut[VA][0], K_VA, pInst);
		p_output_param->m_pDispOut[VA][1] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pDispOut[VA][1], K_VA, pInst);
		p_output_param->m_pDispOut[VA][2] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pDispOut[VA][2], K_VA, pInst);

		p_output_param->m_pCurrOut[VA][0] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pCurrOut[VA][0], K_VA, pInst);
		p_output_param->m_pCurrOut[VA][1] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pCurrOut[VA][1], K_VA, pInst);
		p_output_param->m_pCurrOut[VA][2] = (unsigned char *)dxb_vpu_getFrameBufVirtAddr(p_output_param->m_pCurrOut[VA][2], K_VA, pInst);
...
//			DPRINTF("Displayed addr 0x%x", p_output_param->m_pDispOut[PA][0]);
	...
	else if( iOpCode == VDEC_GET_RING_BUFFER_STATUS )
	{
		...
		ret = dxb_vdec_cmd_process(V_GET_RING_BUFFER_STATUS, &pInst->gsVpuDecBufStatus, pInst); // get the available space in the ring buffer
		if( ret != RETCODE_SUCCESS )
		{
...
//			  );
	...
	else if( iOpCode == VDEC_FILL_RING_BUFFER )
	{
		...
		ret = dxb_vdec_cmd_process(V_FILL_RING_BUFFER_AUTO, &pInst->gsVpuDecBufFill, pInst);  // fille the Ring Buffer
	...
	else if( iOpCode == VDEC_UPDATE_WRITE_BUFFER_PTR )
	{
		...
		ret = dxb_vdec_cmd_process(V_DEC_UPDATE_RINGBUF_WP, &pInst->gsVpuDecUpdateWP,  pInst);  // fille the Ring Buffer
	...
	else if( iOpCode == VDEC_BUF_FLAG_CLEAR )
	{
		...
		ret = dxb_vdec_cmd_process(V_DEC_BUF_FLAG_CLEAR, &idx_display, pInst);
	...
	else if( iOpCode == VDEC_DEC_FLUSH_OUTPUT)
	{
		...
		ret = dxb_vdec_cmd_process(V_DEC_FLUSH_OUTPUT, &pInst->gsVpuDecInOut_Info, pInst);
	...
	{
		ret = dxb_vdec_cmd_process(V_DEC_SWRESET, NULL, pInst);
	...
	{
		ret = dxb_vdec_cmd_process(V_DEC_CLOSE, &pInst->gsVpuDecInOut_Info, pInst);
		if( ret != RETCODE_SUCCESS )
		{
...
void * dxb_vdec_alloc_instance(void)
{
	_vdec_ *pInst = (_vdec_*)TCC_fo_malloc(__func__, __LINE__, sizeof(_vdec_));
	...
	if(pInst)
	{
		...
		{
			ALOGE("%s open error!!", VPU_MGR_NAME);
			goto MGR_OPEN_ERR;
		}
		...
		for ( i=0 ; i<20 ; i++ )
		{
			ioctl(pInst->vpu_mgr_fd, VPU_GET_INSTANCE_IDX, &iInst_info);
			if( iInst_info.nInstance < 0 )
			{
				ALOGE("%s VPU_GET_INSTANCE_IDX error[%d]", __func__, iInst_info.nInstance);
				goto INST_GET_ERR;
			}

			if ( (vdec_open_count == 0) && (iInst_info.nInstance != 0) )
			{
				usleep(100000);		/* wait for 100ms */
				if ( (i+1) < 20 )
				{
					ioctl(pInst->vpu_mgr_fd, VPU_CLEAR_INSTANCE_IDX, &iInst_info);
				}
			}else{
				break;
			}
		...
		pInst->vpu_dec_fd = open(vpu_devices[pInst->vdec_instance_index], O_RDWR);
		...
		vdec_open_count++;
		ALOGI("dxb_vdec_alloc_instance %d, total %d", pInst->vdec_instance_index, vdec_open_count);
	}
...
DEC_OPEN_ERR:
	...
	if( ioctl(pInst->vpu_mgr_fd, VPU_CLEAR_INSTANCE_IDX, &iInst_info) < 0){
		ALOGE("%s ioctl(0x%x) error!!", VPU_MGR_NAME, VPU_CLEAR_INSTANCE_IDX);
	}
INST_GET_ERR:
...
MGR_OPEN_ERR:
	TCC_fo_free(__func__, __LINE__, pInst);
	return NULL;

}

void dxb_vdec_release_instance(void * pInst)
{
	if(pInst)
	{
		_vdec_ * pVdec = (_vdec_ *)pInst;
		...
		ioctl(pVdec->vpu_mgr_fd, VPU_CLEAR_INSTANCE_IDX, &iInst_info);
		...
		if(pVdec->vpu_dec_fd >= 0)
		{
			if(close(pVdec->vpu_dec_fd) < 0)
			{
				DPRINTF("%s close error", vpu_devices[pVdec->vdec_instance_index]);
			}
			pVdec->vpu_dec_fd = -1;
		}

		TCC_fo_free (__func__, __LINE__,pVdec);
		pInst = pVdec = NULL;
		...
		ALOGI("dxb_vdec_release_instance %d, total %d", used_instance, vdec_open_count);
	}
}
...
int dxb_vpu_get_frame_count(int safety_count, void * pInst)
{
	_vdec_ *pVdec = (_vdec_ *)pInst;
