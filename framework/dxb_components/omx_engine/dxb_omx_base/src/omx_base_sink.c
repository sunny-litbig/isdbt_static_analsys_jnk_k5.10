...
*   Description : omx_base_sink.c


*******************************************************************************/
#define LOG_TAG	"OMX_BASE_SINK"
#include <utils/Log.h>
...
OMX_ERRORTYPE dxb_omx_base_sink_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
  ...
  if (openmaxStandComp->pComponentPrivate) {
    ...
    omx_base_sink_Private = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_base_sink_PrivateType));
  ...
  * e.g. we can override the function pointers in the private struct  */
  if(!openmaxStandComp->pComponentPrivate){
  	if(omx_base_sink_Private){
  		TCC_fo_free(__func__, __LINE__, omx_base_sink_Private);  		
  	}
  	return OMX_ErrorInsufficientResources;
  }
  omx_base_sink_Private = openmaxStandComp->pComponentPrivate;

  omx_base_sink_Private->BufferMgmtFunction = dxb_omx_base_sink_BufferMgmtFunction;

  return err;
}

OMX_ERRORTYPE dxb_omx_base_sink_Destructor(OMX_COMPONENTTYPE *openmaxStandComp)
{
...
/** This is the central function for component processing. It
  ...
  */
void* dxb_omx_base_sink_BufferMgmtFunction (void* param) {
