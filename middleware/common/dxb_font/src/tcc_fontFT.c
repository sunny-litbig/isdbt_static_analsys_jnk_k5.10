...
static int g_BaseLineOffset;

#if defined(HAVE_LINUX_PLATFORM)
static char *default_tcc_font_freetype_file_path = "/tccdxb/font";
static char *env_tcc_font_freetype_file_path = NULL;
static char tcc_font_freetype_file_path[128] = {0, };
#endif
/**************************************************************************
							FUNCTION
**************************************************************************/
...
#if defined(HAVE_LINUX_PLATFORM)
	if(tcc_font_freetype_file_path[0] == 0) {
		env_tcc_font_freetype_file_path = getenv("DXB_FONT_PATH");
		if(env_tcc_font_freetype_file_path == NULL) {
			env_tcc_font_freetype_file_path = default_tcc_font_freetype_file_path;
		}

		memset(tcc_font_freetype_file_path, 0x0, 128);
		if( (strlen(env_tcc_font_freetype_file_path) + strlen("/DroidSansFallback_DxB.ttf")) < 128 ) // Noah, To avoid Codesonar's warning, Buffer Overrun
			sprintf(tcc_font_freetype_file_path, "%s%s", env_tcc_font_freetype_file_path, "/DroidSansFallback_DxB.ttf");
	}
