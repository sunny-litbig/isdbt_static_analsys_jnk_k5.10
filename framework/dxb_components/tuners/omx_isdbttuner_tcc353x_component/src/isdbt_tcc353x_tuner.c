...
I32S tcc353x_init(I32S commandInterface, I32S streamInterface)
{
	...
	I32U rfSwitchingUhf[4] =
		...
		(1<<GPIO_NUM_TRIPLE_BAND_RF_SWITCHING_SLAVE1),
		(1<<GPIO_NUM_TRIPLE_BAND_RF_SWITCHING_SLAVE1)};
#elif defined (_HW_TCC8XXX_AK4601_SILAB_ISDBT_DAB_)

	/*	please refer bellows =====================
		Single : GPIO_NUM_RF_SWITCHING_TCC3531_MASTER
		4Diversity :
			master : GPIO_NUM_RF_SWITCHING_TCC3531_MASTER (master's GPIO)
			slave1 : GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1 (master's GPIO)
			slave2 : GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE-1  (slave3's GPIO)
			slave3 : GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE (slave3's GPIO)

		2Diversity + 2Diversity
			[0th module]
			master : GPIO_NUM_RF_SWITCHING_TCC3531_MASTER (master's GPIO)
			slave1 : GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1 (master's GPIO)

			[1st module]
			master (slave2) : GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE-1 (slave3's GPIO)
			slave (slave3) : GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE (slave3's GPIO)
	============================================= */

	/* For Single or 4Diversity */

	I32U rfSwitchingGpioCtrl[4] =
		{(1<<GPIO_NUM_RF_SWITCHING_TCC3531_MASTER) | (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1)),
		0,
		0,
		(1<<(GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE-1)) | (1<<GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE)};
	I32U rfSwitchingVhfLow[4] =  {0,0,0,0};
	I32U rfSwitchingVhfHigh[4] = {0,0,0,0};
	I32U rfSwitchingUhf[4] =
		{(1<<GPIO_NUM_RF_SWITCHING_TCC3531_MASTER) | (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1)),
...
#if 0 /* For Future */
	/* For Dual Mode (2Div + 2Div only) */

		/* Dual-0 */

	I32U rfSwitchingGpioCtrl[4] =
		{(1<<GPIO_NUM_RF_SWITCHING_TCC3531_MASTER) | (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1)),	0, 0, 0};
	I32U rfSwitchingVhfLow[4] =  {0,0,0,0};
	I32U rfSwitchingVhfHigh[4] = {0,0,0,0};
	I32U rfSwitchingUhf[4] =
		{(1<<GPIO_NUM_RF_SWITCHING_TCC3531_MASTER) | (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_MASTER+1)),	0, 0, 0};
	...
	I32U rfSwitchingGpioCtrl[4] =
		{0, (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE-1)) | (1<<GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE), 0, 0};
	I32U rfSwitchingVhfLow[4] =  {0,0,0,0};
	I32U rfSwitchingVhfHigh[4] = {0,0,0,0};
	I32U rfSwitchingUhf[4] =
		{0, (1<<(GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE-1)) | (1<<GPIO_NUM_RF_SWITCHING_TCC3531_SLAVE), 0, 0};

#endif  /* End of For Future */
	...
	/* power control */
	g_tcc_dxb_ctrl_fd = open(ISDBT_CTRL_DEV_FILE, O_RDWR | O_NDELAY);
...
#if defined (_HW_TCC8XXX_AK4601_SILAB_ISDBT_DAB_)
	TcpalPrintLog((I08S *) "[TCC353X] All tuner mode!!!\n");
#endif
...
#if !defined (_USE_TCC353X_TRIPLE_BAND_) && defined (_USE_TCC3531_) && !defined (_HW_TCC8XXX_AK4601_SILAB_ISDBT_DAB_)
		/* default use options */
#else
...
#if defined (_USE_TCC3530_)
	ret = Tcc353xApiInit(0, (I08U *)&TCC353X_BOOT_DATA_ISDBT13SEG[0],
	    TCC353X_BOOT_SIZE_ISDBT13SEG, &streamFormat);
#elif defined (_USE_TCC3532_)
	if(chip_version==VER_TCC3532_0AX) {
		ret = Tcc353xApiInit(0, (I08U *)&TCC353X_BOOT_DATA_ISDBT13SEG_AX[0],
...
I32S tcc353x_get_signal_strength(Tcc353xStatus_t * _monitorValues)
{
	...
	I64U L,H,temp, temp2;
#endif
	memset (&monitor, 0, sizeof (Tcc353xStatus_t));
...
{
	IsdbLock_t lock_status;
	I32S ret;

	ret = Tcc353xApiGetLockStatus(0, 0, &lock_status);
	if (ret == TCC353X_RETURN_SUCCESS) {
		if (lock_status.TMCC != 0) {
			*arg = 1;
		} else {
			*arg = 0;
...
{
	return Tcc353xApiUserLoopStopCmd(_moduleIndex);
}
