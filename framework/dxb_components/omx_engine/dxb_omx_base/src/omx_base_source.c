...
*   Description : omx_base_source.c


*******************************************************************************/ 
#define LOG_TAG	"OMX_BASE_SOURCE"
#include <utils/Log.h>

#include <omxcore.h>
#include <omx_base_source.h>

OMX_ERRORTYPE dxb_omx_base_source_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName) {
  ...
  if (openmaxStandComp->pComponentPrivate) {
    ...
    omx_base_source_Private = TCC_fo_calloc (__func__, __LINE__,1, sizeof(omx_base_source_PrivateType));
  ...
  * e.g. we can override the function pointers in the private struct  */
  if(!openmaxStandComp->pComponentPrivate){
  	if(omx_base_source_Private){
  		TCC_fo_free(__func__,__LINE__, omx_base_source_Private);
  	}
  	return OMX_ErrorInsufficientResources;
  }
  omx_base_source_Private = openmaxStandComp->pComponentPrivate;
  omx_base_source_Private->BufferMgmtFunction = dxb_omx_base_source_BufferMgmtFunction;

  return err;
}

OMX_ERRORTYPE dxb_omx_base_source_Destructor(OMX_COMPONENTTYPE *openmaxStandComp) {
...
/** This is the central function for component processing. It
  ...
  */
void* dxb_omx_base_source_BufferMgmtFunction (void* param) {
  ...
  while(omx_base_component_Private->state == OMX_StateIdle || omx_base_component_Private->state == OMX_StateExecuting ||  
    ...
    DEBUG(DEB_LEV_SIMPLE_SEQ, "Waiting for output buffer semval=%d (%s)\n",pOutputSem->semval, omx_base_source_Private->name);
...
void* dxb_omx_base_source_twoport_BufferMgmtFunction (void* param) {
  OMX_COMPONENTTYPE* openmaxStandComp = (OMX_COMPONENTTYPE*)param;
  omx_base_component_PrivateType* omx_base_component_Private=(omx_base_component_PrivateType*)openmaxStandComp->pComponentPrivate;
  omx_base_source_PrivateType* omx_base_source_Private = (omx_base_source_PrivateType*)omx_base_component_Private;
  ...
  while(omx_base_source_Private->state == OMX_StateIdle || omx_base_source_Private->state == OMX_StateExecuting ||  omx_base_source_Private->state == OMX_StatePause || 
    ...
    if(pOutputSem[0]->semval>0 && isOutputBufferNeeded[0]==OMX_TRUE ) {
      ...
      if(pOutputQueue[0]->nelem>0){
        ...
        pOutputBuffer[0] = dxb_dequeue(pOutputQueue[0]);
    ...
    if(pOutputSem[1]->semval>0 && isOutputBufferNeeded[1]==OMX_TRUE) {
      ...
      if(pOutputQueue[1]->nelem>0){
        ...
        pOutputBuffer[1] = dxb_dequeue(pOutputQueue[1]);
