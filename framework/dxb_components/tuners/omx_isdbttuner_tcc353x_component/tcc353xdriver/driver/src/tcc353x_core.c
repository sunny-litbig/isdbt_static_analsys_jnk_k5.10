...
I32U Tcc353xCoreVersion = ((0<<16) | (1<<8) | (88));

/*                                                                          */
/*--------------------------------------------------------------------------*/

Tcc353xHandle_t Tcc353xHandle[TCC353X_MAX][TCC353X_DIVERSITY_MAX] = { 0, };
...
extern I32U DpllTable_TSB_1SEG_tcc3536[];
extern I32U DpllTable_TSB_3SEG_tcc3536[];
extern I32U DpllTable_TMM_1SEG_tcc3536[];
extern I32U DpllTable_TMM_13SEG_tcc3536[];
extern I32U DpllTable_TMM_USER_1SEG_tcc3536[];
extern I32U DpllTable_TMM_USER_13SEG_tcc3536[];
extern I32U DpllTable_Partial1Seg_tcc3536[];
extern I32U DpllTable_FullSeg_tcc3536[];
...
I32S Tcc353xCheckTcc3531Version(I32S _moduleIndex)
{
	mailbox_t mailbox;
	I32S i;
	I32S skip_flag = 0;
	I08U ver = VER_TCC353X_DONT_CARE;
	I16U plls = PLL_ISDB_T_FULLSEG;

	switch(Tcc353xHandle[_moduleIndex][0].
		options.basebandName) {
	case BB_TCC3530:
		skip_flag = 1;
		break;
	case BB_TCC3531:
	...
	TcpalSemaphoreLock(&Tcc353xOpMailboxSema[_moduleIndex][0]);
	...
	Tcc353xSetRegSysReset(&Tcc353xHandle[_moduleIndex][0],
			      TC3XREG_SYS_RESET_DSP|TC3XREG_SYS_RESET_EP, _LOCK_);
	Tcc353xSetRegSysEnable(&Tcc353xHandle[_moduleIndex][0],
			       TC3XREG_SYS_EN_EP |
			       TC3XREG_SYS_EN_DSP |
			       TC3XREG_SYS_EN_OPCLK |
			       TC3XREG_SYS_EN_RF);
	Tcc353xGetAccessMail(&Tcc353xHandle[_moduleIndex][0]);
	TcpalSemaphoreUnLock(&Tcc353xOpMailboxSema[_moduleIndex][0]);

	Tcc353xMailboxTxRx(&Tcc353xHandle[_moduleIndex][0],
			   &mailbox, MBPARA_SYS_ASM_VER, NULL, 0);

	if((mailbox.data_array[0]>>31) & 0x01) {
		if(Tcc353xHandle[_moduleIndex][0].options.
		    basebandName==BB_TCC3531) {
			ver = VER_TCC3531_2AX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3531 Chip is [2AX] version\n");
		} else if(Tcc353xHandle[_moduleIndex][0].options.
		    basebandName==BB_TCC3532) {
			ver = VER_TCC3532_0AX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3532 Chip is [0AX] version\n");
		} else {
			ver = VER_TCC3531_2AX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3531 Chip is [2AX] version\n");
		...
		if(Tcc353xHandle[_moduleIndex][0].options.
			...
			ver = VER_TCC3531_1XX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3531 Chip is [1XX] version\n");
		} else if(Tcc353xHandle[_moduleIndex][0].options.
		    basebandName==BB_TCC3532) {
			ver = VER_TCC3532_0XX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3532 Chip is [0XX] version\n");
		} else {
			ver = VER_TCC3531_1XX;
			TcpalPrintLog((I08S *)
				"[TCC353X] TCC3531 Chip is [1XX] version\n");
	...
	for (i = 0; i < Tcc353xCurrentDiversityCount[_moduleIndex]; i++)
		Tcc353xHandle[_moduleIndex][i].chipVersion = ver;
		...
		*/
		...
		if(i==0) {
			...
			switch(Tcc353xHandle[_moduleIndex][0].
				options.basebandName) {
			case BB_TCC3530:
				sdram_tmp_data =  0x460;
				break;
			case BB_TCC3531:
				if(Tcc353xHandle[_moduleIndex][i].chipVersion
				    ==VER_TCC3531_2AX)
					sdram_tmp_data =  0x402;
				else
					sdram_tmp_data =  0x460;
				break;
			case BB_TCC3532:
			...
			Tcc353xMiscWrite(_moduleIndex, i, 
				 MISC_SDRAM_REG_CTRL, 0x04, sdram_tmp_data);
				    ...
				    I32U _maxFreqNum, I32U _defaultPll)
{
	...
	for(i = 0; i<_maxFreqNum; i++)	{
		...
		*/
		if(_tables[index] >= _frequencyInfo-1 &&
		   _tables[index] <= _frequencyInfo+1) {
			pll = _tables[index+1];
			break;
		}
	}
...
static I16U Tcc353xGetNewPLL_tcc3536(I32S _moduleIndex, 
    Tcc353xTuneOptions * _tuneOption, I32S _Inputfrequency)
{
	I16U newPll = 0;
	...
	case TCC353X_ISDBT_1_OF_13SEG:
		if(_tuneOption->rfIfType==TCC353X_LOW_IF)
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_Partial1Seg_tcc3536[0],
					_MAX_PARTIAL_1SEG_FREQ_NUM_, 
					PLL_ISDB_T_PARTIAL_1_SEG));
		else
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_FullSeg_tcc3536[0],
					_MAX_PARTIAL_1SEG_FREQ_NUM_, 
					PLL_ISDB_T_PARTIAL_1_SEG));
		break;
	case TCC353X_ISDBT_13SEG:
	...
	case TCC353X_ISDBTMM:
		if (_tuneOption->tmmSet == UserDefine_Tmm13Seg)
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_TMM_USER_13SEG_tcc3536[0],
					_MAX_TMM_USER_13SEG_FREQ_NUM_, 
					PLL_ISDB_TMM_FULLSEG));
		else if (_tuneOption->tmmSet == UserDefine_Tmm1Seg)
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_TMM_USER_1SEG_tcc3536[0],
					_MAX_TMM_USER_1SEG_FREQ_NUM_, 
					PLL_ISDB_TMM_PARTIAL_1_SEG));
		else if (_tuneOption->tmmSet == A_1st_13Seg||
			 _tuneOption->tmmSet == A_2nd_13Seg||
			...
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_TMM_13SEG_tcc3536[0],
					_MAX_TMM_13SEG_FREQ_NUM_, 
					PLL_ISDB_TMM_FULLSEG));
		else
			newPll = (I16U)(Tcc353xSearchDpllValue (
					(I32U)(_Inputfrequency),
					&DpllTable_TMM_1SEG_tcc3536[0],
					_MAX_TMM_1SEG_FREQ_NUM_, 
					PLL_ISDB_TMM_PARTIAL_1_SEG));
		break;
	case TCC353X_ISDBTSB_1SEG:
		 ...
		 Tcc353xTuneOptions * _tuneOption, I32S _fastTune)
{
	...
	if(Tcc353xHandle[_moduleIndex][0].useDefaultPLL == 1)
	{
		if(Tcc353xHandle[_moduleIndex][0].options.basebandName 
		   ...
		   == BB_TCC3536)
			newPll = Tcc353xGetNewPLL_tcc3536
				    (_moduleIndex, _tuneOption, Inputfrequency);
			  ...
			  Tcc353xOption_t * _Tcc353xOption)
{
	...
	case BB_TCC3535:
		...
		break;
	case BB_TCC3536:
		TcpalPrintLog((I08S *) "[TCC353X] TCC3536 selected\n");
		break;
	default:
	...
	for (i = 0; i < Tcc353xCurrentDiversityCount[_moduleIndex]; i++) {
		...
		if(h->options.rfType == TCC353X_TRIPLE_BAND_RF) {
		   	if(h->options.basebandName != BB_TCC3531 && 
		   	   ...
		   	   h->options.basebandName != BB_TCC3536) {
...
static I32S Tcc353xApplySpurSuppression_tcc3536 (I32S _moduleIndex, 
					 Tcc353xTuneOptions *_tuneOption, 
					 I64U *_rcStep, I32U *_adcClkCfg,
					 I32U *_icic, I32U _frequencyInfo)
{
				     ...
				     I32U * _opConfig, I32U _frequencyInfo)
{
	...
	I32U DIV_NUM_CFG = 0;
	I32U SSE = 0; /* apply only isdbt-1segment */
	I32U AGC_TR_SPEED = 3;
	I32U AID= 1;
	...
	if(Tcc353xHandle[_moduleIndex][0].TuneOptions.BandwidthMHz == 8) {
		...
		if(Tcc353xHandle[_moduleIndex][0].options.basebandName 
		   ...
		   == BB_TCC3536)
			Tcc353xApplySpurSuppression_tcc3536(_moduleIndex, 
						    _tuneOption, 
						    &RC_STEP, &ADC_CLK_CFG, 
						    &ICIC, _frequencyInfo);
		else if(Tcc353xHandle[_moduleIndex][0].options.oscKhz == 38400)
	...
	/* opconfig2 DUE is applied by Tcc353xDiversityUnlockEnforce() */
	_opConfig[2] =
	   (DIV_NUM_CFG<<31) | (SSE<<26) | (AID<<25) |
...
I32S Tcc353xDiversityUnlockEnforce(I32S _moduleIndex, 
				   I32S _diversityIndex, 
				   I32S _diversityUnlockEnforcement)
{
	...
	if(Tcc353xCurrentDiversityCount[_moduleIndex]<2) {
		TcpalPrintErr((I08S *)"[TCC353X] Tcc353xDiversityUnlockEnforce Parameter Error\n");
		return TCC353X_RETURN_FAIL;
	}

	OpValue2 = OriginalOpConfig[_moduleIndex][_diversityIndex][2];
	if(_diversityUnlockEnforcement)
		OpValue2 |= 0x40000000;
	else
		OpValue2 &= 0xBFFFFFFF;
	Tcc353xMiscWrite(_moduleIndex, _diversityIndex,
			      MISC_OP_REG_CTRL, TC3XREG_OP_CFG02, OpValue2);
	return ret;
}
