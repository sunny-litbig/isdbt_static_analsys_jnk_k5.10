...
/* skott Add 2005.9.13  end */
   ...
   */
   ...
   DATA_CONTENT_DESCR_ID = 0xC7,		/* 0xC7 Data Content Descriptor (used in ISDB-T, located in EIT)*/
   CA_EMM_TS_DESCR_ID = 0xca,			/*0xCA CA_EMM_TS descriptor(ISDB-T, defined in the NIT)*/
   CA_CONTRACT_INFO_DESCR_ID = 0xcb,    /* 0xCB CA contract information descriptor (used in ISDB-T)  */
   CA_SERVICE_DESCR_ID = 0xcc,			/*0xCC  CA service descriptor (ISDBT, defined int the CAT) */
   ...
   COMPONENT_GROUP_DESCR_ID = 0xd9,     /* 0xD9 component_group_discriptor (used in ISDB-T)              */
   CONTENT_AVAILABILITY_ID = 0xde,
   ACCESS_CONTROL_DESCR_ID = 0xF6,      /* 0xF6 access_control_descriptor (used in ISDB-T)           */
...
/*****************************************************************************
*  Item : CA_EMM_TS descriptor
* Usage : Contains the CA_EMM_TS descriptor
*
*                             MPEG Definition
*                             ===============
*           Syntax                  No. of bits  Mnemonic
...
*	power_supply_period		8	uimsbf
*  }
*
*****************************************************************************/
typedef struct
{
   unsigned char	ucCAEMM_Tag;	
   unsigned char	ucCAEMM_DescLen;	
   unsigned short	uiCAEMM_CASystemID;
   unsigned short	uiCAEMM_TsStreamID;
   unsigned short	uiCAEMM_OrgNetworkID; 	
   unsigned char	ucCAEMM_PowerSuplyPeriod;
}CA_EMM_TS_DESCR;
...
/*****************************************************************************
*  Item : CA_SERVICE_DESCR
* Usage : Contains the CA Service Descriptor
*
*                             MPEG Definition
*                             ===============
*           Syntax                  No. of bits  Mnemonic
...
*     descriptor_length                  8       uimsbf
*     CA_system_ID                     16      uimsbf
*     ca_broadcaster_group_id	8       uimsbf
*	message_control			8       uimsbf	
*     for ( i=0; i<N; i++) {
...
typedef struct
{
   ...
   unsigned short	aucServiceID[CA_PRIVATE_DATA_MAX];   /* 252 */
}CA_SERVICE_DESCR;

/*****************************************************************************
*
*  Item : ACCESS_CONTROL_DESCR
*
* Usage : Contains the access_control_descriptor
*
*
*                             MPEG Definition
*                             ===============
...
*
*****************************************************************************/
typedef struct SUB_DIGITALCOPY_CONTROL_DESC_s
{
	struct SUB_DIGITALCOPY_CONTROL_DESC_s* pNext;
...
typedef struct
{
	...
	SUB_DIGITALCOPY_CONTROL_DESC* sub_info;
 ...
 *  }
 *
 *  additional_data_component_info: This is an 8-bit field and used to extend identifier number 
 *      or storage of supplement information specified in each coding method. Syntax of information 
 *      described in this area is specified otherwise for each data coding method. (see Annex J)
 *****************************************************************************/
#define MAX_A_DATA_COMP_INFO_LENGTH (256-4)  // MAX_DESCRIPTOR_LENGTH - sizeof(descriptor_tag+descriptor_length+data_component_id)
typedef struct
{
	unsigned short  data_component_id;
	unsigned char   DMF;
	unsigned char   Timing;
	unsigned int    a_data_comp_info_length;
	unsigned char   a_data_comp_info[MAX_A_DATA_COMP_INFO_LENGTH];
...
typedef struct SERVICE_ID_LOOP_s
{
	struct SERVICE_ID_LOOP_s* pNext;

	unsigned short		service_id;
} SERVICE_ID_LOOP;

typedef struct TRANSMISSION_TYPE_LOOP_s
{
	struct TRANSMISSION_TYPE_LOOP_s* pNext;

	unsigned char		transmission_type_info;
	unsigned char		num_of_service;

	SERVICE_ID_LOOP*	pServiceIdLoop;
...
typedef struct
{
	...
	TRANSMISSION_TYPE_LOOP* pTransmissionTypeLoop;
} TS_INFO_DESCR;
 ...
 *	}
 *****************************************************************************/
#define MAX_SUPPORT_EVT_GROUP 7
typedef struct {
	unsigned char			group_type;
	unsigned char 			event_count;
	unsigned short			service_id[MAX_SUPPORT_EVT_GROUP];
	unsigned short			event_id[MAX_SUPPORT_EVT_GROUP];
	unsigned short			original_network_id;
	unsigned short			transport_stream_id;
} EVT_GROUP_DESCR;

/*****************************************************************************
 * COMPONENT_GROUP_DESCR_ID
 ...
 *  data. Syntax described in this area is specified otherwise in each data component. (See ARIB B10. Annex J)
 *****************************************************************************/
#define MAX_SELECTOR_LENGTH 128
#define MAX_NUM_OF_LANG 8
#define MAX_NUM_OF_REF 8
#define	TEXT_CHAR_LENGTH 100
typedef struct _data_content_descr{
	unsigned short	data_component_id;
	unsigned char	entry_component;
	unsigned int	selector_length;
	unsigned char   selector_byte[MAX_SELECTOR_LENGTH];
	unsigned int	num_of_language;
	unsigned char	language_tag[MAX_NUM_OF_LANG];
	unsigned char	DMF[MAX_NUM_OF_LANG];
	unsigned int    ISO_639_language_code[MAX_NUM_OF_LANG];	
	unsigned int	num_of_component_ref;
	unsigned char	component_ref[MAX_NUM_OF_REF];
	unsigned int    ISO_639_language_code_i;
	unsigned int	text_length;
	unsigned char	text_char[TEXT_CHAR_LENGTH+1];
} DATA_CONTENT_DESCR;
...
typedef union
{
   ...
   CA_EMM_TS_DESCR		stCAEMMTSD;
   CA_DESCR                   stCAD;
   CA_SERVICE_DESCR		stCASD;
   ...
   DATA_CONTENT_DESCR		stDCD;
   ACCESS_CONTROL_DESCR       stACCD;
} DESCRIPTOR_UNION;
