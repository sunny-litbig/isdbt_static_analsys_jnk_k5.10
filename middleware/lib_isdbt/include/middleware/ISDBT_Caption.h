...
#define ISO639_LANGUAGE_ENGLISH			LANG_CODE('e','n','g')
#define ISO639_LANGUAGE_JAPANESE		LANG_CODE('j','p','n')
#define ISO639_LANGUAGE_PORTUGUESE		LANG_CODE('p','o','r')

/* TMD (Time Control Mode) */
/* Time Control Code - 2 Bits */
#define TMD_FREE							(0x00)
#define TMD_REAL_TIME						(0x01)
#define TMD_OFFSET_TIME						(0x02)
#define TMD_RESERVED						(0x03)
...
typedef struct _isdbt_caption_info
{
	...
	TS_PES_CAP_STATEDATA		State;
	TS_PES_CAP_MGEDATA			Mnge;		
} ISDBT_CAPTION_INFO;
...
}ISDBT_EIT_CAPTION_INFO[MAX_SUPPORT];
...
typedef struct{
	ISDBT_BMP_DATA element[4];
	unsigned int bmp_diff_x;
	unsigned int bmp_diff_y;
}ISDBT_MULTI_BMP_DATA;

typedef struct{
	int bmp_num;
	ISDBT_MULTI_BMP_DATA mbmp[2];
}ISDBT_MULTI_BMP_INFO;
...
typedef struct _isdbt_char_disp_info
{
	...
	ISDBT_MULTI_BMP_INFO multi_bmp;

	double ratio_drcs_x;	//used to scale-up 1seg DRCS
	double ratio_drcs_y;

	unsigned int internal_sound;// internal sound
} T_CHAR_DISP_INFO;
