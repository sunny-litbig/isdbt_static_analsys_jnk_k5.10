...
#define LOG_TAG	"OMX_BASE_PORT"
#include <utils/Log.h>

unsigned char temp_buffer[256*MAX_QUEUE_ELEMENTS] = {0, }; //android
#endif
...
OMX_ERRORTYPE dxb_base_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  ...
  if (!(*openmaxStandPort)) {
    *openmaxStandPort = TCC_fo_calloc (__func__, __LINE__,1,sizeof (omx_base_PortType));
  }
  ...
  if((*openmaxStandPort)->pAllocSem==NULL) {
    (*openmaxStandPort)->pAllocSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
    if((*openmaxStandPort)->pAllocSem==NULL) {
      return OMX_ErrorInsufficientResources;
    }
  ...
  (*openmaxStandPort)->bIsPortFlushed=OMX_FALSE;
	(*openmaxStandPort)->bIsPortChanged=OMX_FALSE;

  /** Allocate and initialize buffer queue */
  if(!(*openmaxStandPort)->pBufferQueue) {
    (*openmaxStandPort)->pBufferQueue = TCC_fo_calloc (__func__, __LINE__,1,sizeof(queue_t));
    if((*openmaxStandPort)->pBufferQueue==NULL) return OMX_ErrorInsufficientResources;
    dxb_queue_init((*openmaxStandPort)->pBufferQueue);
  }
  /*Allocate and initialise port semaphores*/
  if(!(*openmaxStandPort)->pBufferSem) {
    (*openmaxStandPort)->pBufferSem = TCC_fo_calloc (__func__, __LINE__,1,sizeof(tsem_t));
  ...
  (*openmaxStandPort)->PortDestructor = &dxb_base_port_Destructor;
  (*openmaxStandPort)->Port_AllocateBuffer = &dxb_base_port_AllocateBuffer;
  (*openmaxStandPort)->Port_UseBuffer = &dxb_base_port_UseBuffer;
  (*openmaxStandPort)->Port_FreeBuffer = &dxb_base_port_FreeBuffer;
  (*openmaxStandPort)->Port_DisablePort = &dxb_base_port_DisablePort;
  (*openmaxStandPort)->Port_EnablePort = &dxb_base_port_EnablePort;
  (*openmaxStandPort)->Port_SendBufferFunction = &dxb_base_port_SendBufferFunction;
  (*openmaxStandPort)->FlushProcessingBuffers = &dxb_base_port_FlushProcessingBuffers;
  (*openmaxStandPort)->ReturnBufferFunction = &dxb_base_port_ReturnBufferFunction;
  (*openmaxStandPort)->ComponentTunnelRequest = &dxb_base_port_ComponentTunnelRequest;
  (*openmaxStandPort)->Port_AllocateTunnelBuffer = &dxb_base_port_AllocateTunnelBuffer;
  (*openmaxStandPort)->Port_FreeTunnelBuffer = &dxb_base_port_FreeTunnelBuffer;

#if defined(HAVE_LINUX_PLATFORM)
  (*openmaxStandPort)->Port_AllocateBuffer2 = &dxb_base_port_AllocateBuffer2;
  (*openmaxStandPort)->Port_UseBuffer2 = &dxb_base_port_UseBuffer2;
  (*openmaxStandPort)->Port_FreeBuffer2 = &dxb_base_port_FreeBuffer2;
#endif

  return OMX_ErrorNone;
}

OMX_ERRORTYPE dxb_base_port_Destructor(omx_base_PortType *openmaxStandPort){

  if(openmaxStandPort->pAllocSem) {
    tsem_deinit(openmaxStandPort->pAllocSem);
    TCC_fo_free (__func__, __LINE__,openmaxStandPort->pAllocSem);
    openmaxStandPort->pAllocSem=NULL;
  }
  /** Allocate and initialize buffer queue */
  if(openmaxStandPort->pBufferQueue) {
    dxb_queue_deinit(openmaxStandPort->pBufferQueue);
    TCC_fo_free (__func__, __LINE__,openmaxStandPort->pBufferQueue);
    openmaxStandPort->pBufferQueue=NULL;
  }
  /*Allocate and initialise port semaphores*/
  if(openmaxStandPort->pBufferSem) {
    tsem_deinit(openmaxStandPort->pBufferSem);
    TCC_fo_free (__func__, __LINE__,openmaxStandPort->pBufferSem);
    openmaxStandPort->pBufferSem=NULL;
  }

  TCC_fo_free (__func__, __LINE__,openmaxStandPort);
...
OMX_ERRORTYPE dxb_base_port_FlushProcessingBuffers(omx_base_PortType *openmaxStandPort) {
  ...
  if(openmaxStandPort->sPortParam.eDomain!=OMX_PortDomainOther) { /* clock buffers not used in the clients buffer managment function */
    ...
    openmaxStandPort->bIsPortFlushed=OMX_TRUE;
		...
		if(openmaxStandPort->bIsPortChanged )
		{
			openmaxStandPort->bIsPortChanged = OMX_FALSE;
			tsem_signal(omx_base_component_Private->bPortChangeSem);
		}
    ...
    //DEBUG(DEB_LEV_ERR, "In %s %s waiting for flush all condition port index =%d\n",omx_base_component_Private->name, __func__,(int)openmaxStandPort->sPortParam.nPortIndex);
  ...
  while (openmaxStandPort->pBufferSem->semval > 0) {
    DEBUG(DEB_LEV_FULL_SEQ, "In %s Comp %s TFlag=%x Flusing Port=%d,Semval=%d Qelem=%d\n",
    __func__,omx_base_component_Private->name,(int)openmaxStandPort->nTunnelFlags,(int)openmaxStandPort->sPortParam.nPortIndex,
    (int)openmaxStandPort->pBufferSem->semval,(int)openmaxStandPort->pBufferQueue->nelem);

    tsem_down(openmaxStandPort->pBufferSem);
    pBuffer = dxb_dequeue(openmaxStandPort->pBufferQueue);
    if (PORT_IS_TUNNELED(openmaxStandPort) && !PORT_IS_BUFFER_SUPPLIER(openmaxStandPort)) {
      ...
      //DEBUG(DEB_LEV_FULL_SEQ, "In %s: Comp %s is queueing io:%d buffer\n",
      //  __func__,omx_base_component_Private->name,(int)openmaxStandPort->sPortParam.nPortIndex);
  ...
  if (PORT_IS_TUNNELED_N_BUFFER_SUPPLIER(openmaxStandPort)) {
    while(openmaxStandPort->pBufferQueue->nelem!= openmaxStandPort->nNumAssignedBuffers){
	  DEBUG(DEB_LEV_ERR,"%s (%s)-%d Waiting getting a buffer %d %d\n",__func__,omx_base_component_Private->name, (int)openmaxStandPort->sPortParam.nPortIndex, openmaxStandPort->pBufferQueue->nelem, openmaxStandPort->nNumAssignedBuffers);
      tsem_down(openmaxStandPort->pBufferSem);
  ...
  DEBUG(DEB_LEV_FUNCTION_NAME, "Out %s (%s) Port Index=%d\n", __func__, omx_base_component_Private->name, (int)openmaxStandPort->sPortParam.nPortIndex);
 ...
 *
 */
OMX_ERRORTYPE dxb_base_port_DisablePort(omx_base_PortType *openmaxStandPort) {
  omx_base_component_PrivateType* omx_base_component_Private;
  ...
  if(omx_base_component_Private->state!=OMX_StateLoaded) {
    if(!PORT_IS_BUFFER_SUPPLIER(openmaxStandPort)) {
      ...
      while(openmaxStandPort->pBufferQueue->nelem > 0) {
        dxb_dequeue(openmaxStandPort->pBufferQueue);
      }
 ...
 *
 */
OMX_ERRORTYPE dxb_base_port_EnablePort(omx_base_PortType *openmaxStandPort) {
  omx_base_component_PrivateType* omx_base_component_Private;
  OMX_ERRORTYPE err=OMX_ErrorNone;
  OMX_U32 i;
 ...
 */
OMX_ERRORTYPE dxb_base_port_AllocateBuffer(
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
      openmaxStandPort->pInternalBufferStorage[i] = TCC_fo_calloc (__func__, __LINE__,1,sizeof(OMX_BUFFERHEADERTYPE));
      ...
      /* allocate the buffer */
      openmaxStandPort->pInternalBufferStorage[i]->pBuffer = TCC_fo_calloc (__func__, __LINE__,1,nSizeBytes);
 ...
 * bIsFullOfBuffers becomes equal to OMX_TRUE
 */
OMX_ERRORTYPE dxb_base_port_UseBuffer(
  omx_base_PortType *openmaxStandPort,
  ...
  OMX_COMPONENTTYPE* omxComponent = openmaxStandPort->standCompContainer;
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)omxComponent->pComponentPrivate;
  DEBUG(DEB_LEV_FUNCTION_NAME, "In %s (%s)index:%d\n", __func__, omx_base_component_Private->name, nPortIndex);
  if (nPortIndex != openmaxStandPort->sPortParam.nPortIndex) {
    return OMX_ErrorBadPortIndex;
  }
  if (PORT_IS_TUNNELED_N_BUFFER_SUPPLIER(openmaxStandPort)) {
  ...
  if(nSizeBytes < openmaxStandPort->sPortParam.nBufferSize) {
    DEBUG(DEB_LEV_ERR, "In %s: Port %d Given Buffer Size %lu is less than (%s-index%d)Minimum Buffer Size %lu\n", __func__, nPortIndex, nSizeBytes, omx_base_component_Private->name, openmaxStandPort->sPortParam.nPortIndex, openmaxStandPort->sPortParam.nBufferSize);
    return OMX_ErrorIncorrectStateTransition;
  }
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
      ...
      openmaxStandPort->bBufferStateAllocated[i] |= HEADER_ALLOCATED;
      returnBufferHeader = TCC_fo_calloc (__func__, __LINE__,1,sizeof(OMX_BUFFERHEADERTYPE));
 ...
 */
OMX_ERRORTYPE dxb_base_port_FreeBuffer(
  omx_base_PortType *openmaxStandPort,
  OMX_U32 nPortIndex,
  OMX_BUFFERHEADERTYPE* pBuffer) {
    ...
    if (openmaxStandPort->bBufferStateAllocated[i] & (BUFFER_ASSIGNED | BUFFER_ALLOCATED)) {
      ...
      if (openmaxStandPort->bBufferStateAllocated[i] & BUFFER_ALLOCATED) {
        if(openmaxStandPort->pInternalBufferStorage[i]->pBuffer){
          ...
          TCC_fo_free (__func__, __LINE__,openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
          openmaxStandPort->pInternalBufferStorage[i]->pBuffer=NULL;
        }
      } else if (openmaxStandPort->bBufferStateAllocated[i] & BUFFER_ASSIGNED) {
        TCC_fo_free (__func__, __LINE__,pBuffer);
      }
      if(openmaxStandPort->bBufferStateAllocated[i] & HEADER_ALLOCATED) {
        TCC_fo_free (__func__, __LINE__,openmaxStandPort->pInternalBufferStorage[i]);
...
OMX_ERRORTYPE dxb_base_port_AllocateTunnelBuffer(omx_base_PortType *openmaxStandPort,OMX_IN OMX_U32 nPortIndex,OMX_IN OMX_U32 nSizeBytes)
{
  unsigned int i;
  OMX_COMPONENTTYPE* omxComponent = openmaxStandPort->standCompContainer;
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)omxComponent->pComponentPrivate;
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
      pBuffer = TCC_fo_calloc (__func__, __LINE__,1,nSizeBytes);
      if(pBuffer==NULL) {
        return OMX_ErrorInsufficientResources;
      }
      ...
      dxb_queue(openmaxStandPort->pBufferQueue, openmaxStandPort->pInternalBufferStorage[i]);
    }
  }
...
OMX_ERRORTYPE dxb_base_port_FreeTunnelBuffer(omx_base_PortType *openmaxStandPort,OMX_U32 nPortIndex)
{
  unsigned int i;
  OMX_COMPONENTTYPE* omxComponent = openmaxStandPort->standCompContainer;
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)omxComponent->pComponentPrivate;
 ...
 */
OMX_ERRORTYPE dxb_base_port_SendBufferFunction(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE* pBuffer) {
  ...
  if (!PORT_IS_ENABLED(openmaxStandPort) || (PORT_IS_BEING_DISABLED(openmaxStandPort) && !PORT_IS_TUNNELED_N_BUFFER_SUPPLIER(openmaxStandPort)) ||
    ...
    DEBUG(DEB_LEV_ERR, "In %s: Port %d is disabled comp = %s, transientState : %d \n", __func__, (int)portIndex,omx_base_component_Private->name, omx_base_component_Private->transientState);
    return OMX_ErrorIncorrectStateOperation;
  }
 ...
 */
OMX_ERRORTYPE dxb_base_port_ReturnBufferFunction(omx_base_PortType* openmaxStandPort,OMX_BUFFERHEADERTYPE* pBuffer){
...
OMX_ERRORTYPE dxb_base_port_ComponentTunnelRequest(omx_base_PortType* openmaxStandPort,OMX_IN  OMX_HANDLETYPE hTunneledComp,OMX_IN  OMX_U32 nTunneledPort,OMX_INOUT  OMX_TUNNELSETUPTYPE* pTunnelSetup) {
  OMX_ERRORTYPE err = OMX_ErrorNone;
...
#if defined(HAVE_LINUX_PLATFORM)
OMX_ERRORTYPE dxb_base_port_AllocateBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE** pBuffer,
  OMX_U32 nPortIndex,
  OMX_PTR pAppPrivate,
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
	  ...
	  #if 1
	  if(openmaxStandPort->sPortParam.eDir == OMX_DirInput)  //ANdroid
      	openmaxStandPort->pInternalBufferStorage[i]->pBuffer =TCC_fo_calloc (__func__, __LINE__,1,nSizeBytes);
	  else
	  	openmaxStandPort->pInternalBufferStorage[i]->pBuffer = &(temp_buffer[0x100*i]);//i*0x100+0x40980000;
	  //ALOGE("Allodcated Buffer PORT[%d] ADDR[%d]:%08x size:%d\n" ,nPortIndex,i ,openmaxStandPort->pInternalBufferStorage[i]->pBuffer,nSizeBytes);
	  #else
	  openmaxStandPort->pInternalBufferStorage[i]->pBuffer =TCC_fo_calloc (__func__, __LINE__,1,nSizeBytes);
	  #endif
 ...
 * bIsFullOfBuffers becomes equal to OMX_TRUE
 */
OMX_ERRORTYPE dxb_base_port_UseBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE** ppBufferHdr,
  OMX_U32 nPortIndex,
  OMX_PTR pAppPrivate,
 ...
 * When all the bufers are done, the variable bIsEmptyOfBuffers is set to OMX_TRUE
 */
OMX_ERRORTYPE dxb_base_port_FreeBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_U32 nPortIndex,
  OMX_BUFFERHEADERTYPE* pBuffer) {
